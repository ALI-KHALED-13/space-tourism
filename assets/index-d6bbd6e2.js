import{r as c,u as E,j as y,U as C,s as x,m as b,a as v,H as S}from"./index-3cba2d2d.js";const ne=(e="horizontal",t=0)=>{const[s,i]=c.useState(0),a=c.useRef([]),p=E();T({slideInView:s,slideTo:l,carouselRef:a,direction:e,breakPoint:t,screenWidth:p}),c.useEffect(()=>{a.current[1]&&(e=="vertical"&&p>t?(H(a.current[1]),P(a.current[0])):a.current.forEach(o=>{o.style.height="auto",o.style.display="flex"}),setTimeout(()=>l(s),500))},[a.current,p]);function l(o){a.current.forEach(n=>n.scroll({behavior:"smooth",...e==="vertical"&&p>t?{top:o*n.offsetHeight}:{left:o*n.offsetWidth}})),i(o)}const h={width:"100%",overflow:"hidden"},u=c.useCallback(({className:o,children:n})=>y("div",{ref:r=>a.current[0]=r,className:`mediaCarousel ${o||""}`,style:h,children:n}),[]);return{InfoCarousel:c.useCallback(({className:o,children:n})=>y("div",{ref:r=>a.current[1]=r,className:`infoCarousel ${o||""}`,style:h,children:n}),[]),MediaCarousel:u,slideTo:l,slideInView:s}},H=e=>{e.style.display="block";const t=Math.max.apply(null,[...e.children].map(s=>s.offsetHeight));e.style.height=t+"px";for(let s of e.children)s.offsetHeight!==t&&(s.style.marginTop=(t-s.offsetHeight)/1.8+"px",s.style.marginBottom=(t-s.offsetHeight)/1.8+"px")},P=e=>{e.style.display="grid",e.style.gridAutoRows="1fr";const t=[...e.getElementsByTagName("img")].sort((s,i)=>i.naturalHeight-s.naturalHeight)[0];t.complete?e.style.height=e.firstChild.offsetHeight+"px":t.onload=()=>{e.style.height=e.firstChild.offsetHeight+"px"}},oe=e=>{c.useEffect(()=>{const t=document.head.getElementsByTagName("TITLE")[0];return t.textContent+=" | "+e,()=>t.textContent=t.textContent.slice(0,t.textContent.indexOf(" |"))},[])},T=({carouselRef:e,slideInView:t,slideTo:s,direction:i="horizontal",breakPoint:a=0,screenWidth:p=document.documentElement.clientWidth})=>{const[l,h]=c.useState({x:0,y:0}),[u,w]=c.useState({x:0,y:0});c.useEffect(()=>{if(e.current[1]){const n=m=>{const{screenX:d,screenY:f}=m.changedTouches[0];h({x:d,y:f})},r=m=>{const{screenX:d,screenY:f}=m.changedTouches[0];w({x:d,y:f})},g=W(e.current[0],e.current[1]);return g.addEventListener("touchstart",n),g.addEventListener("touchend",r),()=>{g.removeEventListener("touchstart",n),g.removeEventListener("touchend",r)}}},[e.current]);const o=(n,r)=>{r>n+35&&t>0?s(t-1):n>r+35&&t<e.current[0].children.length-1&&s(t+1)};c.useEffect(()=>{if(e.current[1]){const n=u.x-l.x,r=u.y-l.y,g=Math.abs(n)>Math.abs(r)?"horizontal":"vertical";[g,i].every(m=>m==="vertical")&&p>a?o(l.y,u.y):g==="horizontal"&&o(l.x,u.x)}},[u,e.current,p])},W=(e,t)=>{try{if([e,t].some(p=>!(p instanceof Element)))throw new TypeError("one of the passed arguments isn't an element");let s=null,i=e.parentElement,a=t.parentElement;for(;;){if(i===a){s=a;break}i=i.parentElement,a=a.parentElement}return s}catch(s){if(s.name==="typeError")return console.error(s.message),document.body;throw s}},j=C`
  from {
    opacity: 0.5;
    top: -50px;
  }
